...
В задании необходимо просто сделать вызов указанного метода, реальfeatный API не должен интересовать. 

---

### Результаты тестов реализованного класса 

`Чтобы протестировать работу CrptApi, просто запустить класс Main. В нем инициируются запросы из нескольких потоков.`

- **Тест на несуществующем адресе:**
![Testing-on-fake-url.png](Testing-on-fake-url.png)

- **Тест на реальном адресе:**  
Здесь сервер возвращает общую ошибку из-за токена, значение которого выставлено "Bearer YOUR_TOKEN". Подставьте здесь токен.
![Testing-on-real-url.png](Testing-on-real-url.png)

- **Тест сериализации/десериализации экземпляра класса:**
![Testing-serialization-deserialization.png](Testing-serialization-deserialization.png)